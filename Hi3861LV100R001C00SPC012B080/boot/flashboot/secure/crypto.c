...
hi_u32 crypto_check_image_id(hi_u32 image_id)
{
    if (image_id != PRODUCT_UPG_FILE_IMAGE_ID) {
        boot_msg1("[crypto check image id]fail,image id:", image_id);
...
hi_bool crypto_check_kernel_file(hi_u32 addr_start)
{
    ...
    if (ret != HI_ERR_SUCCESS) {
        boot_msg2("[boot check file]flash read err, addr-ret:", addr_start, ret);
        return HI_FALSE;
    }
...
static hi_u32 crypto_encrypt_hash(hi_flash_crypto_content *key_content)
{
    ...
    cs = (uintptr_t)key_content_ptr ^ (content_size - IV_BYTE_LENGTH) ^ (uintptr_t)data_tmp_ptr ^
        (content_size - IV_BYTE_LENGTH);
    ret = memcpy_s(key_content_ptr, content_size - IV_BYTE_LENGTH, data_tmp_ptr, content_size - IV_BYTE_LENGTH, cs);
...
static hi_u32 crypto_encrypt_data(hi_flash_crypto_content *content, encrypt_ctx *para)
{
    hi_u32 ret = HI_ERR_FAILURE;
    hi_cipher_aes_ctrl aes_ctrl;

    hi_u8 *fw_cyp_data = boot_malloc(para->encrypt_total_size);
    ...
    ret = g_flash_cmd_funcs.write(para->kernel_addr + para->encrypt_start_addr, para->encrypt_total_size,
...
hi_u32 crypto_decrypt(hi_u32 ram_addr, hi_u32 ram_size)
{
    ...
    ret = crypto_content_id(&werk_content, &werk_content_bak);
    if (ret != HI_ERR_SUCCESS) {
        return ret;
    }
...
hi_u32 encrypt_para_init(encrypt_ctx *encrypt_para, hi_u32 start)
{
    encrypt_para->para_is_init = HI_TRUE;
    encrypt_para->is_encrypt_section = HI_TRUE;
    encrypt_para->kernel_addr = start;
    encrypt_para->encrypt_start_addr = sizeof(hi_upg_file_head);
    encrypt_para->encrypt_total_size = KERNEL_ENCRYPT_SIZE;
    encrypt_para->encrypt_end_addr = encrypt_para->encrypt_start_addr + encrypt_para->encrypt_total_size;
    encrypt_para->encrypted_size = 0;
    encrypt_para->buf = (hi_u8 *)boot_malloc(KERNEL_ENCRYPT_SIZE);
    if (encrypt_para->buf == HI_NULL) {
        return HI_PRINT_ERRNO_MALLOC_EXAUST_ERR;
    }

    hi_u32 cs = (uintptr_t)(encrypt_para->buf) ^ KERNEL_ENCRYPT_SIZE ^ 0 ^ KERNEL_ENCRYPT_SIZE;
    hi_u32 ret = memset_s(encrypt_para->buf, KERNEL_ENCRYPT_SIZE, 0, KERNEL_ENCRYPT_SIZE, cs);
    if (ret != HI_ERR_SUCCESS) {
        return HI_PRINT_ERRNO_MALLOC_EXAUST_ERR;
    }
...
hi_u32 upg_check_encrypt(encrypt_ctx *para, hi_u8 *buf, hi_u32 buf_len)
{
    hi_u32 ret;
    hi_u32 cs;

    hi_u32 size = para->encrypted_size + buf_len;
    if (size > para->encrypt_total_size) {
        cs = (uintptr_t)(para->buf + para->encrypted_size) ^ (para->encrypt_total_size - para->encrypted_size) ^
            (uintptr_t)buf ^ (para->encrypt_total_size - para->encrypted_size);
        if (memcpy_s(para->buf + para->encrypted_size, para->encrypt_total_size - para->encrypted_size, buf,
            para->encrypt_total_size - para->encrypted_size, cs) != EOK) {
        ...
        ret = hi_flash_write(para->kernel_addr + para->encrypt_end_addr, size - para->encrypt_total_size,
            buf + (para->encrypt_total_size - para->encrypted_size), HI_FALSE);
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }

        para->is_encrypt_section = HI_FALSE;
    } else if (size == para->encrypt_total_size) {
        cs = (uintptr_t)(para->buf + para->encrypted_size) ^ buf_len ^ (uintptr_t)buf ^ buf_len;
        if (memcpy_s(para->buf + para->encrypted_size, buf_len, buf, buf_len, cs) != EOK) {
            return HI_PRINT_ERRNO_CRYPTO_KEY_EMPTY_ERR;
        }
        ...
        if (memcpy_s(para->buf + para->encrypted_size, para->encrypt_total_size - para->encrypted_size,
            buf, buf_len, cs) != EOK) {
            return HI_PRINT_ERRNO_CRYPTO_KEY_EMPTY_ERR;
        }

        para->encrypted_size += buf_len;
    }
...
{
    hi_u32 ret;
    encrypt_ctx *encrypt_para = encrypt_get_ctx();
    ...
    if (encrypt_para->encrypt_start_addr == offset) {
        ret = upg_check_encrypt(encrypt_para, buf, buf_len);
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }
    } else if ((encrypt_para->encrypt_start_addr > offset) &&
               (encrypt_para->encrypt_start_addr < (offset + buf_len))) {
        ret = hi_flash_write(start + offset, (encrypt_para->encrypt_start_addr - offset), buf, HI_FALSE);
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }
        ret = upg_check_encrypt(encrypt_para, buf + encrypt_para->encrypt_start_addr - offset,
            buf_len - (encrypt_para->encrypt_start_addr - offset));
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }
    } else {
    ...
    if ((encrypt_para->is_encrypt_section == HI_FALSE) && (encrypt_para->buf != HI_NULL)) {
...
hi_u32 crypto_kernel_write(hi_u32 start, hi_u32 offset, hi_u8 *buf, hi_u32 buf_len)
{
    ...
    hi_u32 code_offset = sizeof(hi_upg_file_head);

    if (((code_offset >= offset) && (code_offset < (offset + buf_len))) || (para->is_encrypt_section == HI_TRUE)) {
        ret = upg_encrypt_part_write(start, offset, buf, buf_len);
        if (ret != HI_ERR_SUCCESS) {
            boot_msg0("[crypto_kernel_write] encrypt write flash fail!");
        }
    } else {
        ret = hi_flash_write(start + offset, buf_len, buf, HI_FALSE);
        if (ret != HI_ERR_SUCCESS) {
            boot_msg0("[crypto_kernel_write] write flash fail!");
        }
    }
