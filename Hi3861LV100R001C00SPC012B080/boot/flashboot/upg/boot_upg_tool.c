...
{
    hi_flash_read(g_compress_src_addr + offset, size, buffer);
    return 0;
}
...
{
#if defined(CONFIG_FLASH_ENCRYPT_SUPPORT)
    crypto_kernel_write(g_compress_dst_addr, offset, buffer, size);
#else
    hi_flash_write(g_compress_dst_addr + offset, size, buffer, HI_FALSE);
#endif
    return size;
}
...
hi_u32 kernel_comprss_flash_hash(hi_u32 offset, hi_u8 *buffer, hi_u32 size)
{
    ...
    if (offset >= sizeof(hi_upg_file_head)) {
        ...
        if (ret == HI_ERR_SUCCESS) {
            ...
            boot_msg4("[kernel comprss hash]code hash ret-size-offset-headsize", ret, size, offset, hash_size);
        }
        return hash_size;
    }
    ...
    if ((offset + size) > sizeof(hi_upg_file_head)) {
        ...
        if (ret != HI_ERR_SUCCESS) {
            boot_msg4("[kernel comprss hash]head hash updata ret-size-offset-hashsize", ret, size, offset, hash_size);
            return hash_size;
        }
    ...
    if (ret == HI_ERR_SUCCESS) {
        ...
        boot_msg4("[kernel comprss hash]code head hash ret-size-offset-hashsize", ret, size, offset, hash_size);
    }
    return hash_size;
}
...
hi_u32 boot_upg_copy_flash_2_flash(hi_u32 src_addr, hi_u32 src_size, hi_u32 dst_addr, hi_u32 dst_size, hi_bool comprss)
{
    ...
    if (!comprss) {
        boot_msg0("[bootupg decompress]Not support uncompressed file.");
        return ret;
    } else {
        boot_msg4("[bootupg decompress]src-size-dst-size", src_addr, src_size, dst_addr, dst_size);
...
hi_u32 boot_upg_hash_one_content(hi_u32 flash_addr, hi_u32 total_size, hi_u8 *hash, hi_u32 hash_size)
{
    ...
    if (ret != HI_ERR_SUCCESS) {
        boot_msg1("[bootupg hash one content]cipher start ret:", ret);
        return ret;
    }
    ...
    for (uintptr_t offset = 0; offset < total_size;) {
        ...
        if (ret != HI_ERR_SUCCESS) {
            boot_msg1("[bootupg hash one content]flash read ret: ", ret);
        ...
        if (ret != HI_ERR_SUCCESS) {
            boot_msg1("[bootupg hash one content]hash update ret:", ret);
            return ret;
        }
        offset += this_size;
    }
...
hi_u32 boot_upg_hash_more_content(HI_CONST upg_verify_param *param, hi_u8 *hash, hi_u32 hash_size)
{
    ...
    for (hi_u8 i = 0; i < param->buffer_count; i++) {
        ...
        for (uintptr_t offset = 0; offset < size;) {
            ...
            if (ret != HI_ERR_SUCCESS) {
                boot_msg1("[bootupg hash more content]get content ret:", ret);
                (hi_void) hi_cipher_hash_final(hash, hash_size);
                return ret;
            }
