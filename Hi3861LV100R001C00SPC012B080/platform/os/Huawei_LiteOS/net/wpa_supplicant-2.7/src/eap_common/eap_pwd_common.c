			     ...
			     const u8 *token)
{
	...
	if (!cofactor || !grp->pwe || !tmp1 || !pm1 || !one) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: unable to create bignums");
		goto fail;
	}

	if (crypto_ec_cofactor(grp->group, cofactor) < 0) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: unable to get cofactor for "
			   "curve");
		goto fail;
	}
	...
	while (ctr < 40) {
		...
		if (!rnd) {
			wpa_warning_log0(MSG_INFO, "EAP-pwd: unable to create rnd");
			goto fail;
		}
