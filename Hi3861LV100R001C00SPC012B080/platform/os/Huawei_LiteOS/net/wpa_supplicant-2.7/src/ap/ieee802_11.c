...
static void auth_sae_retransmit_timer(void *eloop_ctx, void *eloop_data)
{
	...
	if (sae_check_big_sync(hapd, sta)) {
		if (hapd->conf->mesh & MESH_ENABLED) {
			...
			hi_mesh_report_connect_results(sta, false, WLAN_REASON_MESH_INVALID_PASSWORD);
		}
		return;
	}
	sta->sae->sync++;
				     ...
				     struct sta_info *sta)
{
	if (!(hapd->conf->mesh & MESH_ENABLED))
		return;
	wpa_error_log2(MSG_ERROR, "sae_set_retransmit_timer line %d, hapd->dot11RSNASAERetransPeriod = %d",
		 __LINE__, hapd->dot11RSNASAERetransPeriod);
