	...
	if (hisi_mesh) {
		...
		if (((wpa_s->ifmsh != NULL) || g_mesh_sta_flag) &&
			...
			if (mesh_ap_param.mesh_is_mbr) {
				ret = os_snprintf(pos, end - pos, "[HI_MBR]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
			if (mesh_ap_param.accept_sta) {
				ret = os_snprintf(pos, end - pos, "[HI_ACPT_STA]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
			if (mesh_ap_param.accept_peer) {
				ret = os_snprintf(pos, end - pos, "[HI_ACPT_PEER]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
			ret = os_snprintf(pos, end - pos, "[HI_PPER_NUM%d]", mesh_ap_param.peering_num);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
			ret = os_snprintf(pos, end - pos, "[HI_BCN_PRI%d]", mesh_ap_param.mesh_bcn_priority);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
		}
			   ...
			   char *reply, int reply_size, int *reply_len)
{
		...
		while (pos && *pos != '\0') {
			if (os_strncmp(pos, "ssid ", 5) == 0) {
				...
				if ((end - pos) <= SSID_MAX_LEN) {
					...
					//wpa_error_log0(MSG_ERROR, " wpas_ctrl_scan: wpa_config_parse_string failed");
					*reply_len = -1;
					goto done;
				}
#ifndef HISI_CODE_CROP
	...
	manual_scan_freqs = NULL;
	ssid = NULL;
	if ((os_strstr(reply, "FAIL-BUSY") == NULL) || (*reply_len == -1)) {
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_SCAN_OK);
	}
}
...
static int wpa_supplicant_show_sta(void)
{
	struct sta_info *sta = NULL;
	unsigned int sta_count = 0;
	g_sta_num = g_sta_num < WIFI_DEFAULT_MAX_NUM_STA ? g_sta_num : WIFI_DEFAULT_MAX_NUM_STA;
	if ((g_hapd == NULL) ||
#ifdef LOS_CONFIG_MESH
    (g_mesh_get_sta_flag && (g_mesh_sta_info == NULL)) ||
#endif
    (
#ifdef LOS_CONFIG_MESH
    !g_mesh_get_sta_flag &&
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if ((wpa_s == NULL) || (wifi_dev->iftype == HI_WIFI_IFTYPE_AP)) {
		wpa_printf(MSG_ERROR, "cli cmd with invalid wpa_s, buf = %s", buf);
		goto OUT;
	}
