...
{
	if (!g_mesh_sta_flag || (wpa_s->ifmsh != NULL)) {
		return;
	}
	/* EID + LEN + IE value */
												 ...
												 struct wpa_ssid *group, int only_first_ssid)
{
	...
	for (ssid = group; ssid != NULL; ssid = (only_first_ssid ? NULL : ssid->next)) {
		...
		if (ssid->bssid_set && (ssid->ssid_len == 0) && (os_memcmp(bss->bssid, ssid->bssid, ETH_ALEN) == 0))
			check_ssid = 0;
		if (check_ssid && ((match_ssid_len != ssid->ssid_len) ||
...
static int wpa_mesh_scan(struct wpa_supplicant *wpa_s) {
	...
	const int reply_size = WPA_EXTERNED_SSID_LEN + 1;
	int reply_len;
	char *pos = buf;
...
void hi_wpa_meshap_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (res == 1) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Ignore new mesh scan request for %d.%06d sec since an earlier request is scheduled to trigger sooner",
			sec, usec);
	} else {
...
static void hi_mesh_mpm_send_plink_close_action_put_buf(struct wpa_supplicant *wpa_s, struct wpabuf *buf, const struct mesh_conf *conf)
{
	u8 add_plid;
	u8 ie_len;
	int ampe;
...
int wpa_sta_event_check(enum wpa_event_type event)
{
	return ((event == EVENT_SCAN_RESULTS) ||
			(event == EVENT_RX_MGMT) ||
			(event == EVENT_NEW_PEER_CANDIDATE));
}
