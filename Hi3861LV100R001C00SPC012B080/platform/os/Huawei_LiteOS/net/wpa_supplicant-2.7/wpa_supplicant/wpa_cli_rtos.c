...
#ifdef CONFIG_WPS
int wpa_cli_wps_pbc(struct wpa_supplicant *wpa_s, char *bssid)
{
	char *cmd_bssid_null[] = {"WPS_PBC"};
	char *cmd_bssid[] = {"WPS_PBC", bssid};
...
int wpa_cli_wps_pin(struct wpa_supplicant *wpa_s, char *pin, char *bssid)
{
	...
	if (bssid != NULL) {
		char *cmd_bssid_pin[] = {"WPS_PIN", bssid, pin};
		return wpa_cli_cmd(wpa_s, 3, cmd_bssid_pin);
	} else {
		char *cmd_null_pin[] = {"WPS_PIN any", pin};
		return wpa_cli_cmd(wpa_s, 2, cmd_null_pin);
	}
}
#endif /* CONFIG_WPS */

int wpa_cli_scan(struct wpa_supplicant *wpa_s, char *buf)
{
	char *cmd[] = {"SCAN", buf};
...
int wpa_cli_ap_deauth(struct wpa_supplicant *wpa_s, char *buf)
{
	char *cmd[] = {"AP_DEAUTH", buf};

	return wpa_cli_cmd(wpa_s, 2, cmd);
}

#ifdef LOS_CONFIG_MESH
int wpa_cli_mesh_deauth(struct wpa_supplicant *wpa_s, char *buf)
{
	char *cmd[] = {"MESH_DEAUTH", buf};
...
int wpa_cli_ap_scan(struct wpa_supplicant *wpa_s, char *mode)
{
	char *cmd[] = {"AP_SCAN", mode};

	return wpa_cli_cmd(wpa_s, 2, cmd);
}

int wpa_cli_add_iface(struct wpa_supplicant *wpa_s, char *ifname)
{
	char *cmd[] = {"ADD_IFACE", ifname};
...
int wpa_cli_remove_network(struct wpa_supplicant *wpa_s, char *id)
{
	char *cmd_remove_all[] = {"REMOVE_NETWORK all"};
	char *cmd_remove[] = {"REMOVE_NETWORK", id};
	unsigned int ret;
...
int wpa_cli_select_network(struct wpa_supplicant *wpa_s, char *id)
{
	char *cmd[] = {"SELECT_NETWORK", id};

	return wpa_cli_cmd(wpa_s, 2, cmd);
}

int wpa_cli_set_network(struct wpa_supplicant *wpa_s, char* id, char *param,char *value)
{
	char *cmd[] = {"SET_NETWORK", id, param, value};
...
void wpa_cli_configure_wep(struct wpa_supplicant *wpa_s, char *id, struct wpa_assoc_request *assoc)
{
	char command[33];
	int ret;
...
int wpa_cli_if_start(struct wpa_supplicant *wpa_s, hi_wifi_iftype iftype, char *ifname)
{
	wpa_error_log0(MSG_ERROR, "---> wpa_cli_if_start enter.");
	char *cmd_softap[] = {"SOFTAP_START", ifname};
	char *cmd_wpa[] = {"WPA_START", ifname};
