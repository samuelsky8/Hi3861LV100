 ...
 * @attention
 * <ul>
 * <li>The maximum number of interrupts supported by the Huawei LiteOS Kernel is pre-configured (for details on
 * how to pre-configure an appropriate upper threshold, see the reference document provided by the chip supplier).
 * If the maximum number of interrupts is exceeded, the request fails.</li>
 * <li>The value of registered interrupt handler must not be null. Devices that share a same interrupt ID must each
 * be assigned a unique value of the dev parameter.</li>
 ...
 * <li>The maximum number of interrupts supported by the Huawei LiteOS Kernel is pre-configured (for details on
 * how to pre-configure an appropriate upper threshold, see the reference document provided by the chip supplier).
 * If the maximum number of interrupts is exceeded, the interrupt fails to be deleted.</li>
 ...
 * <li>The maximum number of interrupts supported by the Huawei LiteOS Kernel is pre-configured (for details on
 * how to pre-configure an appropriate upper threshold, see the reference document provided by the chip supplier).
 * If the maximum number of interrupts is exceeded, the interrupt fails to be enabled.</li>
 * <li>Please do make sure the specified irq has corresponding interrupt handler, otherwise, the system would do
 * nothing  but only respond the specified interrupt if the specified interrupt is pending.</li>
 * </ul>
 *
 ...
 * <li>The maximum number of interrupts supported by the Huawei LiteOS Kernel is pre-configured (for details on
 * how to pre-configure an appropriate upper threshold, see the reference document provided by the chip supplier).
 * If the maximum number of interrupts is exceeded, the interrupt fails to be disabled.</li>
 * </ul>
 *
 ...
 *
 * @par Description:
 * This API is used to realize the function of the bottom half of an interrupt handler.
 * The bottom half is executed in handler.
 * @attention
 * <ul>
 ...
 * <li>The memory that is pointed to by work_struct and dynamically requested by this API needs to be released
 * in the bottom half interrupt handler specified by handler. If it is not released, memory leak will result.</li>
 ...
 * @since Huawei LiteOS V100R001C00
 */
bool irq_bottom_half(struct workqueue_struct *work_queue, irq_bottom_half_handler_t handler,
    void *data, unsigned int pri);
#else
/*
 * @ingroup interrupt
 * @brief Bottom half interrupt handler.
