 ...
 * <li>It suggested that calling complete() after wait_for_completion() or
 * wait_for_completion_timeout(), otherwise, wait_for_completion() or wait_for_completion_timeout()
 * would not block because there is already a completion completed.</li>
 * </ul>
 *
 * @param x [IN] Pointer to the completion on which the task to be 
 * woken up is waiting, which must point to valid memory.
 *
 * @retval None.
 * @par Dependency:
 * <ul>
 ...
 * <ul>
 * <li>The input parameter x must point to valid memory, otherwise, the system would be abnormal.</li>
 * <li>Timeout interval timeout should be in [1 , 0xFFFFFFFe],
 * otherwise, it would return OS_WAIT_COMPLETION_ERROR but not wait for completion.</li>
 * <li>Can not be used in interrupt.</li>
 * <li>DO NOT call this API in software timer callback. </li>
 * </ul>
 *
 ...
 * <ul>
 * <li>The input parameter x must point to valid memory, otherwise, the system would be abnormal.</li>
 * <li>It suggested that calling complete_all() after wait_for_completion() or
 * wait_for_completion_timeout(), otherwise, wait_for_completion() or wait_for_completion_timeout()
 * would not block because there is already a completion completed.</li>
 * </ul>
 *
