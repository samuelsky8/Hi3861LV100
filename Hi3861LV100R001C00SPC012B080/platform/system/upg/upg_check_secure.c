    ...
    const hi_upg_common_head *head)
{
    ...
    para->raw_buf = (hi_u8 *)hi_malloc(0, KERNEL_ENCRYPT_SIZE);
    if (para->raw_buf == HI_NULL) {
        return;
    }
    memset_s(para->raw_buf, KERNEL_ENCRYPT_SIZE, 0, KERNEL_ENCRYPT_SIZE);
...
hi_void upg_set_secure_verify_param(upg_verify_param *param, hi_u32 flash_addr, hi_upg_common_head *head)
{
...
{
    hi_u32 ret = upg_secure_verify_head(upg_head);
    if (ret != HI_ERR_SUCCESS) {
        upg_print("[upg check secure info]verify head,ret:0x%x \r\n", ret);
        return ret;
    }

    ret = upg_secure_verify_code(flash_addr, upg_head);
    if (ret != HI_ERR_SUCCESS) {
