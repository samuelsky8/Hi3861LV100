...
hi_u32 upg_set_kernel_size(hi_u32 addr_start)
{
    ...
    hi_u32 ret = hi_flash_read(addr_start + hi_fieldoffset(hi_upg_common_head, file_len), sizeof(hi_u32),
                               (hi_u8 *)(&kernel_size));
    if (ret != HI_ERR_SUCCESS) {
        upg_print("[upg get current kernel size]ret:0x%x \r\n", ret);
        return ret;
    }
...
hi_u32 upg_check_head(hi_void)
{
    ...
    ret = upg_secure_verify_head(upg_head);
...
hi_u32 upg_get_max_file_len(hi_u8 file_type, hi_u32 *file_len)
{
    ...
    if (file_type == HI_UPG_FILE_BOOT) {
        ...
        *file_len = end_addr - start_addr - upg_get_kernel_size();
    }
    ret = HI_ERR_SUCCESS;
#else
    hi_nv_ftm_startup_cfg cfg = { 0 };
...
#if defined(CONFIG_FLASH_ENCRYPT_SUPPORT)
hi_u32 encrypt_para_init(encrypt_ctx *encrypt_para, upg_ctx *ctx)
{
    encrypt_para->para_is_init = HI_TRUE;
    encrypt_para->is_encrypt_section = HI_TRUE;
    encrypt_para->kernel_addr = ctx->file_addr;
    encrypt_para->encrypt_start_addr = sizeof(hi_upg_file_head);
    encrypt_para->encrypt_total_size = KERNEL_ENCRYPT_SIZE;
    encrypt_para->encrypt_end_addr = sizeof(hi_upg_file_head) + encrypt_para->encrypt_total_size;
    encrypt_para->encrypted_size = 0;
    encrypt_para->buf = (hi_u8 *)hi_malloc(0, KERNEL_ENCRYPT_SIZE);
    if (encrypt_para->buf == HI_NULL) {
        return HI_ERR_FLASH_CRYPTO_MALLOC_FAIL;
    }
    memset_s(encrypt_para->buf, KERNEL_ENCRYPT_SIZE, 0, KERNEL_ENCRYPT_SIZE);
    encrypt_para->raw_buf = HI_NULL;
...
hi_u32 upg_check_encrypt(encrypt_ctx *para, hi_u8 *buf, hi_u32 buf_len)
{
    ...
    if (size > para->encrypt_total_size) {
        if (memcpy_s(para->buf + para->encrypted_size, para->encrypt_total_size - para->encrypted_size, buf,
            para->encrypt_total_size - para->encrypted_size) != EOK) {
            return HI_ERR_FLASH_CRYPTO_MEMCPY_FAIL;
        }
        ...
        ret = upg_cache_write(para->encrypt_end_addr, buf + (para->encrypt_total_size - para->encrypted_size),
            size - para->encrypt_total_size);
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }
        ...
        if (memcpy_s(para->buf + para->encrypted_size, buf_len, buf, buf_len) != EOK) {
            return HI_ERR_FLASH_CRYPTO_MEMCPY_FAIL;
        }
...
hi_u32 upg_encrypt_part_write(hi_u32 offset, hi_u8 *buf, hi_u32 buf_len)
{
    hi_u32 ret;
    encrypt_ctx *encrypt_para = encrypt_get_ctx();
    upg_ctx *ctx = upg_get_ctx();
    if (encrypt_para->para_is_init == HI_FALSE) {
        ret =  encrypt_para_init(encrypt_para, ctx);
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }
    }
    ...
    if (encrypt_para->encrypt_start_addr == offset) {
        ...
        ret = upg_cache_write(offset, buf, (encrypt_para->encrypt_start_addr - offset));
        if (ret != HI_ERR_SUCCESS) {
            return ret;
        }
...
hi_u32 hi_upg_transmit(hi_u32 offset, hi_u8 *buf, hi_u32 buf_len)
{
    ...
    ret = upg_encrypt_transmit(ctx, offset, buf, buf_len);
    if (ret != HI_ERR_SUCCESS) {
        encrypt_ctx *para = encrypt_get_ctx();
