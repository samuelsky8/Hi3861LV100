...
u32_t g_weak_up_time = TIMEOUT_MAX;
  ...
  get_next_timeout next_tick)
{
  ...
  if (timeout == NULL) {
    LOWPOER_DEBUG(("sys_timeout_reg: timeout != NULL, pool MEMP_SYS_TIMEOUT is empty"));
    return -1;
  }
...
get_timer_tick(struct timer_entry *t)
{
  ...
  if (t->next_tick != NULL) {
    ...
    tick = 1;
    LOWPOER_DEBUG(("next->tick is NULL\n"));
  }
  tick *= t->clock_max;
  return tick;
}
...
again:
  ...
  sleeptime = sleeptime > sys_timeout_get_weak_time() ? sys_timeout_get_weak_time() : sleeptime;
  set_timer_state(LOW_TMR_TIMER_WAITING, sleeptime);

  sys_timeout_set_weak_time(TIMEOUT_MAX);
  UNLOCK_TCPIP_CORE();
...
sys_timeouts_init(void)
{
  ...
  for (i = 0; i < LWIP_ARRAYSIZE(lowpower_timer_handler); i++) {
    if (set_timer_interval(i, lowpower_timer_handler[i].interval) != 0) {
      LOWPOER_DEBUG(("ERROR:regist timer faild! i = %u\n", i));
    }
  }
}
...
void
sys_timeout_set_weak_time(u32_t val)
{
  g_weak_up_time = val;
}

u32_t
sys_timeout_get_weak_time(void)
{
  return g_weak_up_time;
}
