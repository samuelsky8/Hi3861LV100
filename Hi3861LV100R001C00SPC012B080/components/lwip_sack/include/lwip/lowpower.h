...
#define lowpower_sem_t sys_mutex_t
#define LOWPOWER_SEM_WAIT(lock) sys_mutex_lock(&(lock))
#define LOWPOWER_SINAL(lock) sys_mutex_unlock(&(lock))
#define LOWPOWER_SEM_NEW_LOCK(lock, val)   \
  do {                                     \
    (val) = sys_mutex_new(&(lock));        \
    if ((val) == ERR_OK) {                 \
      sys_mutex_lock(&(lock));             \
    }                                      \
  } while (0)
#define LOWPOWER_SEM_FREE(lock) sys_mutex_free(&(lock))
...
#define LOW_TMR_DELAY_WEAK 20
void sys_timeout_set_weak_time(u32_t val);
u32_t sys_timeout_get_weak_time(void);
