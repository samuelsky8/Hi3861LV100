...
hi_s32 cmd_wpa_mesh_ap_start(hi_s32 argc, const hi_char *argv[])
{
    ...
    if ((integer_check(argv[2]) == HI_ERR_FAILURE) || /* 参数2 */
        ((atoi(argv[2]) != HI_WIFI_SECURITY_OPEN) && (atoi(argv[2]) != HI_WIFI_SECURITY_SAE)) || /* 参数2 */
        ((atoi(argv[2]) == HI_WIFI_SECURITY_OPEN) && (argc != 3))) { /* 参数2为open场景时，固定3个参数个数 */
        return HI_ERR_FAILURE;
    }
    ...
    if (wpa_mesh_assoc_req.auth == HI_WIFI_SECURITY_SAE) {
        buf = argv[3]; /* 3参数 */
        if ((argc != 4) || (strlen(buf) > (HI_WIFI_MS_KEY_LEN_MAX + 2)) || /* 4参数个数  双引号占的2字节 */
            (strlen(buf) < (HI_WIFI_MS_KEY_LEN_MIN + 2))|| /* 2 引号长度 */
            (*buf != '\"') || (*(buf + strlen(argv[3]) - 1) != '\"') || /* 3参数 */
            (memcpy_s((hi_char*)wpa_mesh_assoc_req.key, HI_WIFI_AP_KEY_LEN + 1,
                buf + 1, strlen(argv[3]) - 2) != EOK)) { /* 3参数  双引号占的2字节 */
            return HI_ERR_FAILURE;
        }
    }
...
{
    if (argc != 1) { /* "+SETMESHADV"命令固定1个命令参数 */
        return HI_ERR_FAILURE;
    }
