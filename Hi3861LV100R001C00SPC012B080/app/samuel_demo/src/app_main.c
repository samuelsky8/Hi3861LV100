...
/*************************************************/
#define TASK_PRI_HI 8
#define TASK_PRI_LO 7
#define TASK_PRI 10
hi_u32 demo_task_id = 0;
hi_task_info demo_task_info;

static hi_void* example_task1(hi_void* data)
{
     hi_u32 ret;
     while(1)
     {
         ...
         /*
         demo_task_id = hi_task_get_current_id();
         printf("current task id: %d\r\n", demo_task_id);
         ret = hi_task_get_info(demo_task_id, &demo_task_info);
         if(ret == 0)
         {
             printf("task name: %s\r\n", demo_task_info.name);
         }
...
static hi_void* example_task2(hi_void* data)
{
     hi_u32 ret;
     while(1)
     {
...
hi_void example_task_init(hi_void)
{
    ...
    attr.task_prio = TASK_PRI_HI;
    attr.task_name = (hi_char*)"example_task1";
    attr.task_policy = 1; /*SCHED_FIFO;*/
    attr.task_cpuid = (hi_u32)NOT_BIND_CPU;

    ret = hi_task_create(&demo_task_id, &attr, example_task1, HI_NULL);
    if(ret != HI_ERR_SUCCESS)
    {
        printf("Example_task1 create failed!\n");
    }
    printf("----- example_task1 create success -----\n");
   
     
    attr.task_prio = TASK_PRI_LO;
    attr.task_name = (hi_char*)"example_task2";
    ret = hi_task_create(&demo_task_id, &attr, example_task2, HI_NULL);
    if(ret != HI_ERR_SUCCESS)
    {
        printf("Example_task2 create failed!\n");
    }
    printf("----- example_task2 create success -----\n");
