...
class SconsEnvCfg:
    def __init__(self):
        ...
        self.map_file = os.path.join(self.bin_path, '%s_demo.map'%(CONFIG_TARGET_BIN_NAME))
    ...
    def app_param_update(self, app_name):
        ...
        self.target_name = '%s_%s'%((CONFIG_TARGET_BIN_NAME), self.app_name)
        self.map_file = os.path.join(self.bin_path, '%s_%s.map'%((CONFIG_TARGET_BIN_NAME), self.app_name))

        self.nv_src_path = os.path.join('app', app_name, 'nv')
        self.nv_factory_name = '%s_%s_factory.hnv'%((CONFIG_TARGET_BIN_NAME), app_name)
        self.nv_normal_name = '%s_%s_normal.hnv'%((CONFIG_TARGET_BIN_NAME), app_name)
    ...
    def get_makefile_environs(self, module = 'common'):
        ...
        #makefile_environs['Q'] = ''
        makefile_environs['Q'] = '@'
        makefile_environs['RM'] = 'rm -rf'
        makefile_environs['MAKE'] = 'make'
        makefile_environs['MV'] = 'mv'
        makefile_environs['MAKE_DIR'] = 'mkdir -p'

        # set compiler
        makefile_environs['TOOLS_PREFIX'] = self.tools_prefix
        makefile_environs['GCC_VER_NUM'] = self.gcc_ver_num
        makefile_environs['CC'] = '%sgcc'%self.tools_prefix
        makefile_environs['AR'] = '%sar'%self.tools_prefix
        makefile_environs['AS'] = '%sas'%self.tools_prefix
        makefile_environs['CXX'] = '%scpp'%self.tools_prefix
        makefile_environs['LINK'] = '%sld'%self.tools_prefix
        makefile_environs['RANLIB'] = '%sranlib'%self.tools_prefix
        makefile_environs['OBJCOPY'] = '%sobjcopy'%self.tools_prefix
        makefile_environs['OBJDUMP'] = '%sobjdump'%self.tools_prefix
        makefile_environs['LIBPATH'] = '-L%s -L%s -L%s'%(' -L'.join(os_lib_path), self.lib_path, self.link_path)
        makefile_environs['LIBS'] = '-l%s'%(' -l'.join(self.get_libs()))
        makefile_environs['BASE_NUM'] = self.base_sum

        # Set input/output folders
        makefile_environs['LOG_PATH'] = self.log_path
        makefile_environs['OBJ_PATH'] = self.obj_path
        makefile_environs['LIB_PATH'] = self.lib_path
        makefile_environs['CACHE_PATH'] = self.cache_path
        makefile_environs['LINK_PATH'] = self.link_path
        makefile_environs['NV_PATH'] = self.nv_path
        makefile_environs['BIN_PATH'] = self.bin_path

        makefile_environs['TARGET_NAME'] = self.target_name
        makefile_environs['NV_CFG_NAME'] = self.nv_cfg_name
