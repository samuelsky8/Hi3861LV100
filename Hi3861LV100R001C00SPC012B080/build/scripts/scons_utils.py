...
def condition_str(macro, macro_val, macro_cfg, env_type, depends = None):
    if isinstance(macro_val, str):
        if depends is None:
            return 'ifeq ($(%s), %s)\n\t%s\nendif\n'%(macro, macro_val, translate_env_value_to_str(env_type, macro_cfg))
        else:
            (macro_2, macro_2_val) = depends
            return 'ifeq ($(%s), %s) and ($(%s), %s)\n\t%s\nendif\n'%(macro, macro_val, macro_2, macro_2_val, translate_env_value_to_str(env_type, macro_cfg))
    else:
        strs = ''
        val_list = deepcopy(macro_val)
        val_list.remove('others') if 'others' in val_list else None
        strs = '%sifeq ($(%s), %s)\n\t%s\n'%(strs, macro, val_list[0], translate_env_value_to_str(env_type, macro_cfg[val_list[0]]))
        for val in val_list[1:]:
            strs = '%selse ifeq ($(%s), %s)\n\t%s\n'%(strs, macro, val, translate_env_value_to_str(env_type, macro_cfg[val]))
        strs = '%selse\n\t%s\n'%(strs, translate_env_value_to_str(env_type, macro_cfg['others'])) if 'others' in macro_val else None
        strs = '%sendif\n'%strs
        return strs
