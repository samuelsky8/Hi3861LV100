...
typedef enum {
    ...
    HI_SYSERR_EID_SYS_HARD_REBOOT,      /**< system hard reboot.CNcomment:系统硬复位.CNend */
...
typedef struct {
    ...
    hi_u32 power_magic; /**< power down magic number.CNcomment:下电魔术字.CNend */
...
/**
* @ingroup  iot_crash_info
* @brief Retains the PC pointer when the watchdog reset. CNcomment:看门狗复位时记录PC值。CNend
*
* @par 描述:
*        Retains the PC pointer when the watchdog reset.
CNcomment:看门狗复位时记录PC值，打开此功能后看门狗复位有复位不完全的风险，量产版本请务必关闭。CNend
*
* @attention None
* @param  enable          [IN] type #hi_bool ，Enable/Disable retains reset PC pointer function.
CNcoment:打开/关闭记录复位PC值功能。CNend
*
* @retval None
* @par 依赖:
*           @li hi_crash.h：Describes Crash log APIs. CNcomment:文件用于描述死机相关接口。CNend
