 ...
 * @ingroup hi_wifi_basic
 *
 * Length of psk.CNcomment:psk的长度.CNend
 */
...
typedef enum {
    ...
    HI_WIFI_SECURITY_WPAPSK,                /**< 认证类型:WPAPSK */
...
typedef struct {
    ...
    unsigned int wps_session : 1;           /**< WPS标识 PBC-0/PIN-1 */
...
*                CNcomment:4. <ifname>和<len>由调用者申请内存.CNend
...
*             <ap_list>size最大为：sizeof(hi_wifi_ap_info ap_list) * 64.CNend \n
*             2. ap_num: parameters can be passed in to specify the number of scanned results.The maximum is 64. \n
*             CNcomment:2. ap_num: 可以传入参数，指定获取扫描到的结果数量，最大为64。CNend \n
...
* @param  period        [IN]    Type #unsigned int reconnect period in seconds.CNcomment:重连间隔周期.CNend
* @param  max_try_count [IN]    Type #unsigned int max reconnect try count number.CNcomment:最大重连次数.CNend
*
* @retval #HISI_OK        Execute successfully.
* @retval #HISI_FAIL      Execute failed.
* @par Dependency:
...
*                CNcomment:6. <ifname>和<len>由调用者申请内存.CNend
*             7. SSID only supports ASCII characters. \n
*                CNcomment:7. SSID 只支持ASCII字符.CNend
...
*
* @attention  1. parameter of sta_num max value is 6.CNcomment:1. sta_num 最大值是6.CNend\n
...
*
* @attention  NULL
* @param  addr             [IN]     Type  #const char *, station mac address.CNcomment:MAC地址.CNend
* @param  addr_len         [IN]     Type  #unsigned char, station mac address length.CNcomment:MAC地址长度.CNend
*
* @retval #HISI_OK        Execute successfully.
* @retval #HISI_FAIL      Execute failed.
* @par Dependency:
...
*           CNcomment:设置nd offload打开、并且设置相应ipv6地址，或者设置nd offload关闭.CNend
*
* @attention  NULL
* @param  ifname          [IN]     Type  #const char *, device name.
...
/**
* @ingroup  hi_wifi_basic
* @brief  Set sta pm mode.CNcomment:设置STA的FAST_PS、PSPOLL_PS、uapsd低功耗模式.CNend
*
* @par Description:
*           Set sta pm mode.CNcomment:设置STA的FAST_PS、PSPOLL_PS、uapsd低功耗模式.CNend
*
* @attention  1.CNcomment:mode范围是0~2, 0:FAST_PS; 1:PSPOLL_PS; 2:uapsd.CNend\n
*             2.CNcomment:本API需要在STA start之后，且在sta CONN关联之前调用.CNend
* @param  mode    [IN]  Type  #unsigned char, pm mode.CNcomment:具体的低功耗模式,0:FAST_PS; 1:PSPOLL_PS; 2:uapsd.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
* @par Dependency:
